# AutoRechercheAlternance -- AI Job Search Pipeline
...
- `{"status": "update-available", "local": "1.0.0", "remote": "1.1.0", "changelog": "..."}` → tell the user:
  > "AutoRechercheAlternance update available (v{local} → v{remote}). Your data (CV, profile, tracker, reports) will NOT be touched. Want me to update?"
...
The user can also say "check for updates" or "update AutoRechercheAlternance" at any time to force a check.
To rollback: `node update-system.mjs rollback`

## What is AutoRechercheAlternance
...
> - Run `/AutoRechercheAlternance scan` (or `/AutoRechercheAlternance-scan` if using OpenCode) to search portals
> - Run `/AutoRechercheAlternance` to see all commands
>
...
If the user accepts, use the `/loop` or `/schedule` skill (if available) to set up a recurring `/AutoRechercheAlternance scan` (or `/AutoRechercheAlternance-scan` if using OpenCode). If those aren't available, suggest adding a cron job or remind them to run `/AutoRechercheAlternance scan` (or `/AutoRechercheAlternance-scan` if using OpenCode) periodically.
